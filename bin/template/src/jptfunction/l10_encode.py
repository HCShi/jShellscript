#!/usr/bin/python3
# coding: utf-8
# python2 和 python3 编码不一样, python3 中统一 unicode, 瞬间感觉简单了好多
...
# list 格式化输出
for a, b, c in list(zip(lis[::3], lis[1::3], lis[2::3])):  # 不足的是最后面的可能不是整倍数
    print('{!s:<10}{!s:<20}{!s:<}'.format(a, b.encode('utf-8'), c.encode('utf-8')))  # 每行 3 个打印, format 只支持 utf-8(str)
import sys, pprint; pprint.pprint(sys.path, indent=4)  # 另一种自动格式化的输出方法
...
import chardet; print(chardet.detect('中国'.encode('utf-8')))  # 查看编码, str 对象又很多编码, unicode 是对象, 不是 str 的编码
