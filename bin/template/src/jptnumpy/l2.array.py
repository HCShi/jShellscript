#!/usr/bin/python
# coding: utf-8
from numpy import *
x = arange(0, 10, 0.5); print x
mask = (5 < x) * (x < 7.5)
print mask  # matirx with False and True
print x[mask]
A = array([[n + m * 10 for n in range(5)] for m in range(5)])

##################################################################
print "# where 使用 where 函数能将索引掩码转换成索引位置"
##################################################################
indices = where(mask)
print indices  # 使用 where 函数能将索引掩码转换成索引位置

##################################################################
print "# 使用 diag 函数能够提取出数组的对角线"
##################################################################
print A
print diag(A)  # 对角线
print diag(A, -1)  # 对角线下面的一条斜线

##################################################################
print "# take 函数与高级索引（fancy indexing）用法相似 但是 take 也可以用在 list 和其它对象上"
##################################################################
v2 = arange(-3, 3)
print v2
row_indices = [1, 3, 5]
print v2[row_indices]
print v2.take(row_indices)
take([-3, -2, -1, 0, 1, 2], row_indices)

##################################################################
print "# choose 选取多个数组的部分组成新的数组"
##################################################################
which = [1, 0, 1, 0]
choices = [[-2, -2, -2, -2], [5, 5, 5, 5]]
print choose(which, choices)  # array([ 5, -2,  5, -2])

##################################################################
print "# 线性代数"
# 矢量化是用 Python/Numpy 编写高效数值计算代码的关键，这意味着在程序中尽量选择使用矩阵或者向量进行运算，比如矩阵乘法等
##################################################################
##################################################################
print "# 标量运算"
##################################################################
v1 = arange(0, 5)
print v1 * 2
print v1 + 2
print A * 2, A + 2

##################################################################
print "# Element-wise(逐项乘) 数组-数组 运算"
##################################################################
print A * A  # 对应项相乘
print v1 * v1
print A.shape, v1.shape
print A * v1

##################################################################
print "# 矩阵代数"
print "矩阵乘法, 两种方法"
##################################################################
##################################################################
print "# 1.使用 dot 函数进行 矩阵－矩阵，矩阵－向量，数量积乘法"
##################################################################
print dot(A, A)  # 跟 A * A 不同
print dot(A, v1)  # v1: 0, 1, 2, 3, 4
print dot(v1, v1)  # 会自动转置
##################################################################
print "# 2.将数组对象映射到 matrix 类型"
##################################################################
M = matrix(A)
v = matrix(v1).T  # make it a column vector, 转置
print v
print M * M
print M * v
print v.T
print v.T * v
print v + M * v

##################################################################
print "# 转置"
##################################################################
v = matrix([1, 2, 3, 4, 5, 6])
print shape(v), shape(v.T)

##################################################################
print "# 数组/矩阵 变换"
##################################################################
C = matrix([[1j, 2j], [3j, 4j]])
print C
print conjugate(C)  # 共轭矩阵
print C.H  # 共轭转置

##################################################################
print "# real 与 imag 能够分别得到复数的实部与虚部"
##################################################################
print real(C)
print imag(C)

##################################################################
print "# angle 与 abs 可以分别得到幅角和绝对值"
##################################################################
print angle(C + 1)
print abs(C)

##################################################################
print "# 矩阵计算"
##################################################################
##################################################################
print "# 矩阵求逆"
##################################################################
from scipy.linalg import *
print inv(C)  # 等价于 C.I
print C.I * C

##################################################################
print "# 行列式"
##################################################################
print linalg.det(C)
print linalg.det(C.I)

##################################################################
# 数据处理: 从这里开始还没整理, 懒...
##################################################################

