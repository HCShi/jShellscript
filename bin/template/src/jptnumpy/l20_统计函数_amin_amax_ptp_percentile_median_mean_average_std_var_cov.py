...
##################################################################
## numpy.ptp(a, axis=None, out=None); 函数返回沿轴的值的范围 (最大值 - 最小值)
a = np.array([[3, 7, 5], [8, 4, 3], [2, 4, 9]]); print(a)  # [[3 7 5] [8 4 3] [2 4 9]]
...
# 计算方差
a = np.array([1, 2, 3, 4])
print(a - a.mean(), (a-a.mean())*(a-a.mean()))  # [-1.5 -0.5  0.5  1.5] [ 2.25  0.25  0.25  2.25]
##################################################################
## 协方差
print(np.cov([[1, 2], [1, 4]]))  # [[ 0.5  1.5] [ 1.5  4.5]]; 没有 axis 参数
print(np.cov([[1, 2], [1, 4]], rowvar=0))  # [[ 0.  0.] [ 0.  2.]]; rowvar=0, 说明传入的数据一行代表一个样本, 这个真变态, 和 axis=0 的默认不同
##################################################################
## 总结:
